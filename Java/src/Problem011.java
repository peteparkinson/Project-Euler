 ...
 * 20×20 grid?
 * 
 * answer = 70600674;  //correct
 */

public class Problem011 {
	
	static int product = 1;
	static int[][] number = new int[20][20];
	
	public static void main(String[] args) throws Exception {
		
		File file = new File("./Problem011.txt");
		Scanner sc = new Scanner(file);
		
		while (sc.hasNextLine()){
			for (int y = 0; y < number.length; y++) {
				for (int x = 0; x < number.length; x++) {
					number[y][x] = Integer.parseInt(sc.next());
				}
			}
		}		
	...
	private static void checkDiag(int x, int y) {
		...
		//only need to check up diagonals
		//down diagonal checks would all be redundant
